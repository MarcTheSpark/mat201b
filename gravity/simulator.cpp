...
struct LinInterp {
  ...
  void target(float t) { currentTarget = t; progress = 0; }
  void time(float t) { progressIncrement = 1.0 / (SAMPLE_RATE * t); progress = 0; }

  LinInterp(float initialValue = 0, float transitionTime = BLOCK_SIZE) {
    lastTarget = currentTarget = initialValue;
    progress = 1;
    time(transitionTime);
  }
...
struct GravitySimulator : App {
  ...
  GravitySimulator() : st(440), lint(400, 3) {
    lint.target(600);
