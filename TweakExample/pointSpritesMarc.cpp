...
Modified:
Marc Evans, Jan. 2017 to add in some random variation to the texture, point positions,
and the point opacities.
...
public:
	...
	Mesh data;
	Texture spriteTex;
	uniform_real_distribution<> pointOpacityVariation, positionVariation, textureRandomVariation;
  	random_device rd;
  	mt19937 e2;

	MyApp() :
	e2(rd()),
	pointOpacityVariation(0, 1),
	positionVariation(0.5, 2),
	textureRandomVariation(0, 1),
	spriteTex(16, 16, Graphics::LUMINANCE, Graphics::FLOAT)
	{
		...
		for(int i=0; i<N; ++i){ float x = float(i)/(N-1)*2-1;
			data.vertex(x*positionVariation(e2),y*positionVariation(e2),z*positionVariation(e2));
			data.color(x*0.1+0.1, y*0.1+0.1, z*0.1+0.1, pointOpacityVariation(e2));
		}}}
		...
		for(int i=0; i<Nx; ++i){ float x = float(i)/(Nx-1)*2-1;
			...
			pixels[j*Nx + i] = m * textureRandomVariation(e2);
		}}
